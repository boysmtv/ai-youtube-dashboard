...
export default async function DashboardPage() {
  ...
  const systemAlerts: Array<{ label: string; reason: string; nextAction: string }> = [];
  if (!engineHealth || engineHealth.status !== "ok") {
    systemAlerts.push({
      label: "Service perlu perhatian",
      reason: "Engine utama belum merespons dengan status aman.",
  ...
  return (
    <AppShell>
      ...
      <section className="mt-6 ta-panel p-5">
        ...
        <div className="mt-4">
          {safeSystem ? (
            <div className="rounded-2xl border border-success-200 bg-success-50 px-4 py-3 text-sm text-success-900">Sistem siap berjalan.</div>
          ) : (
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {systemAlerts.slice(0, 6).map((alert) => (
                <div key={`${alert.label}-${alert.reason}`} className="rounded-2xl border border-warning-200 bg-warning-50 p-4 text-sm text-warning-900">
                  <strong className="block">{alert.label}</strong>
                  <p className="mt-2">{alert.reason}</p>
                  <p className="mt-2 text-xs text-warning-800">Langkah berikutnya: {alert.nextAction}</p>
                </div>
              ))}
            </div>
          )}
