...
export function ApprovalList({ approvals }: Readonly<{ approvals: ApprovalAudit[] }>) {
  return (
    <div className="grid gap-3">
      {approvals.length === 0 ? (
        <div className="ta-empty">Belum ada riwayat persetujuan upload.</div>
      ) : (
        approvals.map((approval) => (
          <article key={approval.id} className="ta-panel p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="ta-label text-brand-600">{approval.job_id ? `Video #${approval.job_id}` : "Operasi"}</p>
            ...
            <p className="mt-4 text-gray-700">{approval.action || "Approval diterima."}</p>
            <p className="mt-2 text-gray-700">{approval.approval_reason || "Belum ada alasan."}</p>
            {approval.payload_json ? <p className="mt-2 text-xs text-gray-500">Payload approval tersimpan untuk audit.</p> : null}
