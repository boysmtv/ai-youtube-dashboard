...
import { getJobSummary } from "../../../../../../lib/engine-api";
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const payload = await getJobSummary(jobId, stateView);
    return NextResponse.json(payload);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to load job summary.";
